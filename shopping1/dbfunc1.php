...
function add_pro_to_bill($bid,$uid,$it_id,$pprice,$noqty,$tamt,$status,$date,$cartid)
	{
		$conn=connection();	
		$sql="INSERT into bill values ('$bid','$uid','$it_id','$pprice','$noqty','$tamt','$status','$date','$cartid')";
		echo $sql."<br>";
		$nor=mysqli_query($conn,$sql);
...
function get_it_by_cartid($cartid)
{
	$conn=connection();
	$sql="select item_id from bill where cart_id='$cartid'";
	echo $sql."<br>";
	$result=$nor=mysqli_query($conn,$sql);
	$item=array();$i=0;
	while($row=mysqli_fetch_assoc($result)) 
	{
		$item[$i++]=$row['item_id'];
