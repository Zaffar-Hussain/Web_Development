...
if (isset($_REQUEST['showcart']))
{
   ...
   // $it_id=array();
    ...
    foreach($products as $product)
    {
        ...
        echo $str1;
        $buydet[$product[0]]=$product[3];
       // $it_id[$i++]=$Product[1];
        $noqty=array($product[2]);
         
         $tamt=array($product[4]);
         $it_id=array($product[1]);
...
$_SESSION['tamt']=$tamt;
    ...
    echo "<input type='submit' name=buyitem value=Buy>";
        ...
        echo "hi1";
    {
        ...
        foreach ($ids as $cartid => $price) 
        {
            ...
            if(isset($_REQUEST[$chname]))
            {
                ...
                $date=date('Y-m-d');
                $uid=$_SESSION['id'];
                if(add_pro_to_bill($bid,$uid,$it_id,$pprice,$noqty,$tamt,$status,$date,$cartid)==true)
                {
